...
- **Campos que son richText hoy**: `home`/`homeEn`.`intro` y `.closingText`; `pageFerrol`/`pageFerrolEn`.`heroText`, `.arrivalAddressText`, `.arrivalTransportText`, `.closingText`; `pageIrse`/`pageEn`.`heroText`/`.closingText`; `pageNosotros`/`pageNosotrosEn`.`heroText`, `.historiaParagraphs` (antes array de strings, ahora un único campo richText — cada párrafo que escribe el editor separado por Intro es un bloque), `.closingText`; `pageMentores`.`heroText`/`.whyText`/`.closingText`; `faq.answer`; `testimonial.quote`.
